...
if __name__ == '__main__':
    ...
    pf.equities['GOOG'].plot(ax=ax, ldt_timestamps=ldt_timestamps, column='nml_close')
    pf.plot(ax=ax, ldt_timestamps=ldt_timestamps)
    bo = pf.equities['GOOG'].bollinger_band(tick="GOOG", window=20, nml=True)
    ax.plot(ldt_timestamps, bo['mi'])
    ax.plot(ldt_timestamps, bo['up'])
    ax.plot(ldt_timestamps, bo['lo'])
    legend = ['XOM', "Portfolio", "XOM mid", "XOM up", "XOM lo"]
    ax.legend(legend, loc=2)
    fig.autofmt_xdate()
    pdf_file = order_file + '.pdf'
    fig.savefig(pdf_file, format='pdf')
    beta, alpha = pf.beta_alpha(pf.equities['$SPX'])
